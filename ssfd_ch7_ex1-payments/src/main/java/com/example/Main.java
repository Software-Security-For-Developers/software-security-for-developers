...
public class Main {
    public static void main(String[] args) {
        try {
            // 1) Load EC private key (PKCS#8) from resources
            PrivateKey privateKey = EccKeyLoader.loadPrivateKeyFromPem("/private_key.pem");

            // 2) Load encrypted data from resources (refunds.json.ecc)
            ...
            // 3) Decrypt using ECIES (single step)
            ...
            System.err.println("Failed to decrypt refunds.json.ecc: " + e.getMessage());
